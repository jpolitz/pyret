 ...
 *
 *   node run.js --env=cpo|embed|embed-static|vscode|vscode-ovsx [--compiler=pyret|ts|direct|ts-promise|ts-hybrid] [--grep=<regex>] [--suites=all|a,b] [--reporter=spec|tap|dot]
 *
 * Examples:
...
const USAGE = "usage: node run.js --env=cpo|embed|embed-static|vscode|vscode-ovsx [--compiler=pyret|ts|direct|ts-promise|ts-hybrid] [--grep=<regex>] [--suites=all|a,b] [--reporter=spec|tap|dot]";
...
if (!["pyret", "ts", "direct", "ts-promise", "ts-hybrid"].includes(compiler)) {
  console.error("--compiler must be pyret, ts, direct, ts-promise, or ts-hybrid");
  process.exit(2);
}
