...
before(async () => {
  ...
  if (wantCompiler === "ts" || wantCompiler === "ts-promise" || wantCompiler === "ts-hybrid") {
  ...
  if (wantCompiler === "ts-hybrid") {
    // The proof the hybrid machine is really in play: the runtime exposes it
    // (R.$vm) and the loaded builtins carry bytecode programs.
    const hasVm = await page.eval("(function(){ try { var r = window.THE_RUNTIME || (window.pyRuntime); return !!(r && r.$vm); } catch(e) { return false; } })()");
    if (!hasVm) {
      console.log("[note] could not confirm R.$vm from the page globals; relying on CPO_COMPILER");
    }
  }
