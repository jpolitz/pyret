...
function PYRET_PAGE_ASSERTIONS() {
  const PA = {
    ...
    // flake).
    editorReady() {
      // Editors predating the EDITOR_CONTENTS_SETTLED contract (they set
      // neither flag) are ready when Pyret has loaded -- the mocha util.js
      // criterion this port was validated against.
      const settled = window.EDITOR_CONTENTS_SETTLED === true
        || (window.EXPECTS_HOST_RESET === undefined && window.EDITOR_CONTENTS_SETTLED === undefined);
      return PA.pyretLoaded() && PA.cmPresent() && settled;
    },
