            ...
            if(isNumber(next)) { finishVal(reprMethods["number"](next)); }
              ...
              if(typeof objHasBeenSeen === "string") {
                ...
                if (s && typeof s.then === "function") {
                  // Async _output: propagate the Promise up so toReprFun
                  // can resume the state machine with the resolved value.
                  return s;
                }
        ...
        while(true) {
          ...
          case 0:
            ...
            if ($ans && typeof $ans.then === "function") {
              // toReprHelp suspended on an async _output method.  Resume
              // the state machine when the promise resolves by handing it
              // back through a freshly built ActivationRecord.
              return $ans.then(function(resolved) {
                var resumeAR = new ActivationRecord(
                  "torepr async resume", toReprFun, 1, resolved, [], []);
                return toReprFun(resumeAR);
              });
            }
            return $ans;
          case 1:
      ...
      function reenterToReprFun(val) {
        ...
        while(true) {
          ...
          case 0:
            ...
            if(isContinuation($ans)) { break; }
            if ($ans && typeof $ans.then === "function") {
              // toReprFun suspended on a Promise (async _output method).
              // Pop the stack-of-stacks once the promise resolves and
              // return the value.
              return $ans.then(function(resolved) {
            ...
            if (curPair !== false) {
              ...
              if (isRef(curLeft) && isRef(curRight)) {
                ...
                if (!sameBrands(getBrands(curLeft), getBrands(curRight))) {
                  ...
                  if (newAns && typeof newAns.then === "function") {
                    // Async _equals method: propagate the Promise up so
                    // equalFun resumes the worklist with the resolved value.
                    return newAns;
                  }
      ...
      function equalFun($ar) {
        ...
        while(true) {
          ...
          case 0:
            ...
            if ($ans && typeof $ans.then === "function") {
              // Async _equals method suspended.  Resume the worklist
              // when the promise resolves.
              return $ans.then(function(resolved) {
                var resumeAR = new ActivationRecord(
                  stackFrameDesc, equalFun, 1, resolved, [], []);
                return equalFun(resumeAR);
              });
            }
            return $ans;
          case 1:
            ...
            if ($ans && typeof $ans.then === "function") {
              return $ans.then(function(resolved) {
                for (var i = 0; i < toCompare.stack.length; i++) {
                  ...
                  if (current.setCache) {
                    cache.equal[current.index - 1] = resolved;
                  }
                }
...
//      this.refinement = ann.refinement || this.refinement;
    ...
    */
    ...
    // safeCall in runtime-async knows that fun() and after() can return
    // Promises (the runtime calls user code which is async).  If a Promise
    // is returned, we deferr the rest of the chain until it resolves and
    // return a Promise to our caller (who should also be async-aware in
    // runtime-async, either as a literal async function or by awaiting).
      ...
      while(!skipLoop) {
        ...
        case 0:
          ...
          if ($ans && typeof $ans.then === "function") {
            ++thisRuntime.GAS;
            return $ans.then(function(resolved) {
              var afterRes = after(resolved);
              return afterRes; // may itself be a Promise; caller's await unwraps
            });
          }
          continue;
        case 1:
    ...
    var eachLoop = function eachLoop(fun, start, stop) {
      var i = start;
      return iterDualMode("eachLoop", function() {
        if (i >= stop) { return { kind: "done", val: thisRuntime.nothing }; }
        var idx = i++;
        return { kind: "call", fn: fun.app, args: [idx],
                 then: function(res) { /* ignored */ } };
      });
    };
          ...
          try {
            ...
            while(theOneTrueStackHeight > 0) {
              ...
              if (val && typeof val.then === "function") {
                // The frame returned a Promise (runtime-async path: e.g.
                // safeCall propagated an async user function's pending
                // result, or pauseStack returned a Promise directly).
                // Suspend the trampoline until the promise settles, then
                // continue iter with the resolved value.
                thisThread.pause();
                val.then(
                  function(resolved) {
                    thisThread.handlers.resume(resolved);
                  },
                  function(err) {
                    thisThread.handlers.error(err);
                  }
                );
                return;
              }
              if(next.fun instanceof Function && isContinuation(val)) {
...
//            console.error("Exceptions should no longer be thrown: ", e);
    ...
    // pauseStack in runtime-async always returns a Promise.  The iter
    // trampoline below was extended to detect Promise frame returns and
    // await them, so the bootstrap chain (runStandalone -> safeCall) and
    // async user code converge on the same Promise-based suspension
    // mechanism.
      ...
      return new Promise(function(resolve, reject) {
        try {
          resumer({
            ...
            break: function() {
              reject(thisRuntime.ffi && thisRuntime.ffi.userBreak);
            }
          });
        } catch (e) {
          reject(e);
    ...
    // ------------------------------------------------------------------
    // Async-backend bridge.
    //
    // The compiled async user code is one big `async function` (the
    // toplevel-fun emitted by anf-loop-compiler.arr with --async-backend).
    // Wrap-modules emits `R.runAsyncToplevel(toplevel, module_load, label)`
    // instead of `R.safeCall(...)`.  runAsyncToplevel calls toplevel(),
    // awaits the resulting promise via pauseStack (so the outer
    // runStandalone -> safeCall chain that drives module loading is
    // suspended cleanly), then resumes with module_load(value).
    //
    // GAS/RUNGAS are forced to Infinity for the async run because the
    // compiled async code skips the gas-check prologue (every await is its
    // own yield) and JS builtins called from async user code must not emit
    // Cont markers (the user code can't unwind through `await`).
    // ------------------------------------------------------------------

    function runAsyncToplevel(asyncFn, after, label) {
      var savedGAS = thisRuntime.GAS;
      var savedRunGAS = thisRuntime.RUNGAS;
      thisRuntime.GAS = Infinity;
      thisRuntime.RUNGAS = Infinity;
      function restoreGAS() {
        thisRuntime.GAS = savedGAS;
        thisRuntime.RUNGAS = savedRunGAS;
      }
      var asyncResult;
      try {
        asyncResult = asyncFn();
      ...
      if (!asyncResult || typeof asyncResult.then !== "function") {
        restoreGAS();
        return after(asyncResult);
      }
      // Hand a Promise back up the chain.  safeCall in this runtime is
      // Promise-aware and iter() pauses-and-awaits on Promise frame
      // returns, so the bootstrap converges on the same machinery.
    ...
    // execThunk: dual-mode `run-task`.  Called from BOTH async-backend
    // compiled code (which `await`s a Promise<Either>) AND non-async-
    // backend compiled code (which expects a sync trampoline-style
    // return — value or Cont, never Promise).  Uses safeCall as the
    // dispatch layer: safeCall handles Cont (push AR, return Cont) and
    // Promise (return $ans.then(after)) on the user thunk's return.
    // Sync throws and async rejections are packaged into a sentinel
    // ExecThunkErr instance that the after-handler converts to Right.
    function ExecThunkErr(err) { this.err = err; }
    function execThunk(thunk) {
      ...
      function wrapErr(e) {
        var exn;
        if (isPyretException(e)) { exn = e; }
        else if (e && e.stack) {
          exn = makePyretFailException(thisRuntime.ffi.makeMessageException(String(e) + "\n" + e.stack));
        } else {
          exn = makePyretFailException(thisRuntime.ffi.makeMessageException(String(e)));
        }
        return exn;
      ...
      return thisRuntime.safeCall(
        function() {
          var res;
          try { res = thunk.app(); }
          catch (e) { return new ExecThunkErr(e); }
          if (res && typeof res.then === "function") {
            return res.then(undefined, function(e) { return new ExecThunkErr(e); });
          }
          return res;
        },
        function(result) {
          if (result instanceof ExecThunkErr) {
            var exn = wrapErr(result.err);
            if (isPyretException(exn) && thisRuntime.ffi.isUserBreak(exn.exn)) {
              throw exn;
            }
            return thisRuntime.ffi.makeRight(makeOpaque(exn));
          }
    ...
    */
    ...
    var raw_array_build = function raw_array_build(f, len) {
      ...
      var arr = new Array();
      var i = 0;
      return iterDualMode("raw-array-build", function() {
        if (i >= len) { return { kind: "done", val: arr }; }
        var idx = i++;
        return { kind: "call", fn: f.app, args: [idx],
                 then: function(res) { arr.push(res); } };
      });
    };

    var raw_array_build_opt = function raw_array_build_opt(f, len) {
      ...
      var arr = new Array();
      var i = 0;
      return iterDualMode("raw-array-build-opt", function() {
        if (i >= len) { return { kind: "done", val: arr }; }
        var idx = i++;
        return { kind: "call", fn: f.app, args: [idx],
                 then: function(res) {
    ...
    var raw_array_fold = function raw_array_fold(f, init, arr, start) {
      ...
      var i = 0;
      return iterDualMode("raw-array-fold", function() {
        if (i >= length) { return { kind: "done", val: acc }; }
        var idx = i++;
        return { kind: "call", fn: f.app, args: [acc, arr[idx], idx + start],
                 then: function(res) { acc = res; } };
      });
    };
    ...
    var raw_array_bool_mapper = function(name, good, bad) {
      return function(f, arr, start) {
        ...
        var i = start;
        var earlyOut = null;
        return iterDualMode(name, function() {
          if (earlyOut !== null) { return { kind: "done", val: earlyOut }; }
          if (i >= length) { return { kind: "done", val: good }; }
          var idx = i++;
          return { kind: "call", fn: f.app, args: [arr[idx], idx],
                   then: function(res) { if (res === bad) { earlyOut = res; } } };
        });
      };
    };
    ...
    var raw_array_map = function raw_array_map(f, arr) {
      ...
      var i = 0;
      return iterDualMode("raw-array-map", function() {
        if (i >= length) { return { kind: "done", val: newArray }; }
        var idx = i++;
        return { kind: "call", fn: f.app, args: [arr[idx]],
                 then: function(res) { newArray[idx] = res; } };
      });
    };

    var raw_array_each = function raw_array_each(f, arr) {
      ...
      var length = arr.length;
      var i = 0;
      return iterDualMode("raw-array-each", function() {
        if (i >= length) { return { kind: "done", val: nothing }; }
        var idx = i++;
        return { kind: "call", fn: f.app, args: [arr[idx]],
                 then: function(res) { /* ignored */ } };
      });
    ...
    var raw_array_mapi = function raw_array_mapi(f, arr) {
      ...
      var newArray = new Array(length);
      var i = 0;
      return iterDualMode("raw-array-mapi", function() {
        if (i >= length) { return { kind: "done", val: newArray }; }
        var idx = i++;
        return { kind: "call", fn: f.app, args: [arr[idx], idx],
                 then: function(res) { newArray[idx] = res; } };
    ...
    // ------------------------------------------------------------------
    // dual-mode iteration helper.
    //
    // The raw-list-* / raw-array-* / eachLoop helpers are called from
    // both async-backend Pyret code (where every Pyret call is `await
    // fn.app(...)`) AND from non-async-backend Pyret code (where the
    // sync trampoline expects either a value or a Cont marker).  A
    // user fn passed in returns:
    //   - a sync value, in both modes;
    //   - a Cont marker, only when called from non-async code that ran
    //     out of RUNGAS; never from async-backend user code (which has
    //     RUNGAS = Infinity);
    //   - a Promise, only when called from async-backend user code;
    //     never from non-async code.
    // So the two non-value return shapes are mutually exclusive, and
    // we can write a single helper that handles all three.
    //
    // iterDualMode(name, body) runs `body` (a function) repeatedly.
    // body is a *state machine step*: it returns one of
    //   {kind: "done", val: V}              -- finished, return V
    //   {kind: "call", fn, args, then}      -- call fn.apply(null, args);
    //                                          then(result) continues the
    //                                          machine.
    // body is allowed to mutate its closed-over state between calls.
    // ------------------------------------------------------------------
    function iterDualMode(name, body) {
      var pendingThen = null;
      function loopHelp() {
        while (true) {
          // Skip RUNGAS check entirely.  The user fn we call may return
          // a Promise (async-backend) or a Cont (RUNGAS-exhausted sync)
          // — those cases are handled below.  We do not need our own
          // gas check, because:
          //   - For async-backend callers, gas is Infinity and each
          //     user await yields to the event loop on its own.
          //   - For sync trampoline callers, the user fn returning Cont
          //     is sufficient to trigger our resumption-via-AR path.
          var step = body();
          if (step.kind === "done") { return step.val; }
          // step.kind === "call"
          var res = step.fn.apply(null, step.args);
          if (res && typeof res.then === "function") {
            pendingThen = step.then;
      ...
      function loopFun($ar) {
        if (thisRuntime.isInitializedActivationRecord($ar) && pendingThen !== null) {
          pendingThen($ar.ans);
          pendingThen = null;
        }
        var res = loopHelp();
    ...
    var raw_list_map = function raw_list_map(f, lst) {
      ...
      var currentAcc = [];
      var currentLst = lst;
      return iterDualMode("raw-list-map", function() {
        if (!thisRuntime.ffi.isLink(currentLst)) {
          return { kind: "done", val: thisRuntime.ffi.makeList(currentAcc) };
        }
        var fst = thisRuntime.getColonField(currentLst, "first");
        currentLst = thisRuntime.getColonField(currentLst, "rest");
        return { kind: "call", fn: f.app, args: [fst],
                 then: function(res) { currentAcc.push(res); } };
      });
    };


    var raw_list_join_str_last = function raw_list_join_str_last(lst, sep, lastSep) {
      ...
      var currentAcc = [];
      var currentLst = lst;
      return iterDualMode("raw-list-join-str-last", function() {
        if (!thisRuntime.ffi.isLink(currentLst)) {
          if (currentAcc.length <= 1) { return { kind: "done", val: currentAcc.join(sep) }; }
          var lastElem = currentAcc.pop();
          return { kind: "done", val: currentAcc.join(sep) + lastSep + lastElem };
        }
        var fst = thisRuntime.getColonField(currentLst, "first");
        currentLst = thisRuntime.getColonField(currentLst, "rest");
        return { kind: "call", fn: tostring.app, args: [fst],
                 then: function(res) { currentAcc.push(res); } };
      });
    };
     ...
     */
    var raw_array_map1 = function raw_array_map1(f1, f, arr) {
      ...
      var newArray = new Array(length);
      var i = 0;
      return iterDualMode("raw-array-map1", function() {
        if (i >= length) { return { kind: "done", val: newArray }; }
        var idx = i++;
        var toCall = idx === 0 ? f1 : f;
        return { kind: "call", fn: toCall.app, args: [arr[idx]],
                 then: function(res) { newArray[idx] = res; } };
      });
    };

    var raw_list_filter = function raw_list_filter(f, lst) {
      ...
      var currentAcc = [];
      var currentLst = lst;
      return iterDualMode("raw-list-filter", function() {
        if (!thisRuntime.ffi.isLink(currentLst)) {
          return { kind: "done", val: thisRuntime.ffi.makeList(currentAcc) };
        }
        var fst = thisRuntime.getColonField(currentLst, "first");
        ...
        return { kind: "call", fn: f.app, args: [fst],
                 then: function(res) {
                   if (!isBoolean(res)) {
                     thisRuntime.ffi.throwNonBooleanCondition(["raw-list-filter"], "Boolean", res);
                   }
                   if (isPyretTrue(res)) { currentAcc.push(fst); }
                 } };
      });
    };

    var raw_array_filter = function raw_array_filter(f, arr) {
      ...
      var newArray = [];
      var i = 0;
      return iterDualMode("raw-array-filter", function() {
        if (i >= length) { return { kind: "done", val: newArray }; }
        var idx = i++;
        return { kind: "call", fn: f.app, args: [arr[idx]],
                 then: function(res) {
                   if (!isBoolean(res)) {
                     thisRuntime.ffi.throwNonBooleanCondition(["raw-array-filter"], "Boolean", res);
                   }
                   if (isPyretTrue(res)) { newArray.push(arr[idx]); }
                 } };
      });
    };

    // Original step-machine body retained as dead code so the subsequent
    // `var ...` declarations parse with the same surrounding scope.
    var _unused_raw_array_filter_old = function(f, arr) {
      var currentIndex = -1;
      var length = arr.length;
    ...
    var raw_list_fold = function raw_list_fold(f, init, lst) {
      ...
      var currentAcc = init;
      var currentLst = lst;
      return iterDualMode("raw-list-fold", function() {
        if (!thisRuntime.ffi.isLink(currentLst)) {
          return { kind: "done", val: currentAcc };
        }
        var fst = thisRuntime.getColonField(currentLst, "first");
        currentLst = thisRuntime.getColonField(currentLst, "rest");
        return { kind: "call", fn: f.app, args: [currentAcc, fst],
                 then: function(res) { currentAcc = res; } };
      });
    };
    // The original step-machine raw_list_fold body below is unreachable; we
    // keep the lexical structure so the subsequent `var raw_*` definitions
    // continue to parse without re-indentation churn.
    var _unused_raw_list_fold_old = function(f, init, lst) {
      var currentAcc = init;
      var currentLst = lst;
      function foldHelp() {
    ...
    // Runtime-async makeVariantConstructor: write the constructor as a plain
    // JS function (no eval/Function.apply JIT).  The original runtime built
    // the constructor body as a JS source string and evaluated it via
    // `new Function`; we just write it directly here.  Flat-only annotations
    // produce a sync constructor (so that JS callers like `throwArityErrorC`
    // get back a real Pyret value, not a Promise).  If any annotation is
    // non-flat (e.g. a user refinement), the constructor is async and uses
    // checkConstructorArgs2 to thread checks through `await`.
    function makeVariantConstructor(
      ...
      // Compute checkArg -> position-in-allArgs once
      // (allArgs is the array of argument names, checkArgs is a subset).
      var checkArgPositions = checkArgs.map(function(a) {
        return allArgs.indexOf(a);
      });

      function buildTheFun() {
        var argNames = constructor.$fieldNames;
        var checkAnns = checkAnnsThunk();
        var flat = true;
        for (var i = 0; i < checkAnns.length; i++) {
          if (!checkAnns[i].flat) { flat = false; break; }
        }
        var Construct = thisRuntime.makeDataTypeConstructor(
          reflName, reflFields, allArgs.length, allMuts, constructor, false, loc);
        var arity = allArgs.length;

        function buildDict(args) {
          var dict = thisRuntime.create(base);
          for (var i = 0; i < arity; i++) {
            if (allMuts[i]) {
              var ck = checkArgs.indexOf(allArgs[i]);
              if (ck >= 0) {
                dict[argNames[i]] = thisRuntime.makeUnsafeSetRef(checkAnns[ck], args[i], checkLocs[ck]);
              } else {
                dict[argNames[i]] = thisRuntime.makeUnsafeSetRef(thisRuntime.Any, args[i], loc);
              }
            } else {
              dict[argNames[i]] = args[i];
            }
          }
          return new Construct(dict, brands);
        }

        if (flat) {
          // Sync constructor: inline _checkAnn calls.
          return function() {
            if (arguments.length !== arity) {
              thisRuntime.checkConstructorArityC(loc, reflName, arity, cloneArgs.apply(null, arguments));
            }
            for (var ci = 0; ci < checkArgs.length; ci++) {
              var pos = checkArgPositions[ci];
              var v = arguments[pos];
              if (checkMuts[ci]) {
                if (!thisRuntime.isGraphableRef(v)) {
                  thisRuntime._checkAnn(checkLocs[ci], checkAnns[ci], v);
                }
              } else {
                thisRuntime._checkAnn(checkLocs[ci], checkAnns[ci], v);
              }
            }
            return buildDict(arguments);
          };
        } else {
          // Async constructor: await checkConstructorArgs2 then build dict.
          return async function() {
            if (arguments.length !== arity) {
              thisRuntime.checkConstructorArityC(loc, reflName, arity, cloneArgs.apply(null, arguments));
            }
            var args = cloneArgs.apply(null, arguments);
            var checkVals = checkArgPositions.map(function(p) { return args[p]; });
            return await thisRuntime.checkConstructorArgs2(
              checkAnns, checkVals, checkLocs, checkMuts,
              async function() { return buildDict(args); });
          };
        }
      }

      var funToReturn = makeFunction(function() {
        var theFun = buildTheFun();
        funToReturn.app = theFun;
        return theFun.apply(null, arguments);
      }, reflName);
