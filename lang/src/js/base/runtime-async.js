...
//            console.error("Exceptions should no longer be thrown: ", e);
    ...
    */
    ...
    // Maybe-promise raw-array helpers (same resumable-loop discipline as the
    // equality drain): a NON-async loop that returns the result synchronously
    // when nothing suspends, and a resumable Promise only on a fuel pause or a
    // suspending callback. The loop's whole continuation is (backing array,
    // index), so a suspension returns `<thenable>.then(resume-from-index)`.
    // Cooperative yield preserved exactly.
    ...
    // Maybe-promise raw-list-map (see the raw_array_* loops): a NON-async loop
    // that returns the built List synchronously when nothing suspends, and a
    // resumable Promise only when it pauses for fuel or the callback suspends.
    // The loop's entire continuation is just (remaining list, accumulator), so
    // a suspension returns `<thenable>.then(() => resume(rest, acc))`.
    // Cooperative yield preserved exactly.
    ...
    // Maybe-promise raw-list-fold (see raw_list_map_loop). The continuation is
    // (acc, remaining list); fold's own (f, acc, lst) shape IS that
    // continuation, so a suspension resumes by re-calling the loop. On a
    // callback suspend the resolved value becomes the next accumulator.
