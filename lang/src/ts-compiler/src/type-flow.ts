...
interface Ctx {
  ...
  // generalizes the old Any-always var rule to ANY type: a counter's `+` result stays
  // Number, a string accumulator's stays String, etc.
...
// During a var-ub fixpoint pass, fold one of var `key`'s value sources (its init
...
function absOfLettable(e: N.ALettable, ctx: Ctx): AbsType {
  ...
  if (N.isAApp(e)) {
    ...
    if (name !== undefined) {
      ...
      // Everything else (`num-*`, `string-*`, `tostring`, `raise`, ...):
...
// Also tags each `obj.meth(args)` whose receiver/method pair passes directMethodOk
// for direct `obj.dict["meth"].full_meth(obj, args)` dispatch. An ANF -> ANF
// rewrite, so (like weakenOperators) it MUST run before the identity-keyed
// pre-passes (makeProgMethodInfo) and codegen.
