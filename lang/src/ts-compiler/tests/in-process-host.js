...
function makeStandaloneSource(src, uri, context, options) {
  const finder = (ctx, dep) => CML.moduleFinder(ctx, dep);
  const loc = CL.stringLocator(uri, src);
  const ws = CL.compileWorklistKnownModules(finder, loc, context, new Map());
...
function makeHost(opts) {
  ...
  return new Promise((resolve, reject) => {
    requirejs(
      ...
      function (runtimeLib, loadHooksLib, _stackLib) {
        try {
          ...
          const bootStandalone = makeStandaloneSource(
