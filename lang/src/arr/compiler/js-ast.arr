...
data JExpr:
  ...
  # NOTE(async-backend): an `async function`. Purely additive; the trampoline
  # backend never emits this node.
  | j-fun-async(id :: String, name :: String, args :: CList<A.Name>, body :: JBlock) with:
    method label(self): "j-fun-async" end,
    method print-ugly-source(self, printer) block:
  ...
  # NOTE(async-backend): `(await <exp>)`. Purely additive.
  | j-await(exp :: JExpr) with:
    ...
    method tosource(self):
      PP.surround(INDENT, 1, PP.str("(await "), self.exp.tosource(), PP.str(")"))
    end
...
default-map-visitor = {
  ...
  method j-fun-async(self, id, name, args, body): j-fun-async(id, name, args, body.visit(self)) end,
  method j-await(self, exp): j-await(exp.visit(self)) end,
