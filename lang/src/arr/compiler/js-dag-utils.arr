...
fun used-vars-jexpr(e :: J.JExpr, so-far :: NameSet) -> NameSet:
  cases(J.JExpr) e block:
    ...
    | j-fun(id, _, args, body) =>
      ...
      so-far
    # An async function has identical free-variable structure to a plain function;
    # the `async` keyword only affects evaluation, not scoping. We use the same
    # body-keyed memo as j-fun so async/non-async functions with byte-identical
    # bodies share cached results, which matters for the bootstrap re-compile.
    | j-async-fun(id, _, args, body) =>
      ...
      so-far
    | j-await(expr) => used-vars-jexpr(expr, so-far)
    | j-new(func, args) =>
