...
(define (function #:contract [contract #f] #:args [args #f]
  ...
  (when examples
    ; '◊{...} in old .poly.pm files produces a raw list of strings, not a txexpr
    (when (and (list? examples) (not (null? examples)) (string? (car examples)))
      (set! examples `(pre ([class "pyret-highlight"]) ,@examples)))
